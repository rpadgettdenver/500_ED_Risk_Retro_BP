...
class BusinessDevelopmentExporter:
    ...
    def export_high_value_targets(self):
        ...
        # FIXED: Removed reference to non-existent property_type_clean column
        query = f"""
        WITH building_details AS (
            SELECT 
                ...
                oda.building_name,
                oda.property_type,
                oda.property_type as property_type_clean,  -- FIXED: Just use property_type
        ...
        # Then merge with EPB data - keep track of EPB columns
        print("🔗 Merging EPB data...")
        epb_cols = epb_lookup[['Building ID', 'is_epb', 'epb_approved']].copy()
        results_df = results_df.merge(
            epb_cols,
            left_on='building_id',
            right_on='Building ID',
        ...
        if missing_targets.any():
            ...
            # Clean up the EPB_Final_Target column
            if 'EPB_Final_Target' in results_df.columns:
                results_df.drop('EPB_Final_Target', axis=1, inplace=True)
        
        # Clean up duplicate Building ID columns
        building_id_cols = [col for col in results_df.columns if col.startswith('Building ID') and col != 'building_id']
        if building_id_cols:
            results_df.drop(building_id_cols, axis=1, inplace=True)
        
        # Handle is_epb and epb_approved columns - check if they exist before filling
        if 'is_epb' not in results_df.columns and 'is_epb_from_epb' in results_df.columns:
            results_df['is_epb'] = results_df['is_epb_from_epb']
            results_df.drop('is_epb_from_epb', axis=1, inplace=True)
        
        if 'epb_approved' not in results_df.columns and 'epb_approved_from_epb' in results_df.columns:
            results_df['epb_approved'] = results_df['epb_approved_from_epb']
            results_df.drop('epb_approved_from_epb', axis=1, inplace=True)
        
        # Fill NaN values for EPB columns
        if 'is_epb' in results_df.columns:
            results_df['is_epb'] = results_df['is_epb'].fillna(False)
        else:
            results_df['is_epb'] = False
            
        if 'epb_approved' in results_df.columns:
            results_df['epb_approved'] = results_df['epb_approved'].fillna(False)
        else:
            results_df['epb_approved'] = False
...
def main():
    ...
    print("🏢 ENERGIZE DENVER - HIGH VALUE BUILDING EXPORT (V3 FIXED)")
    print("=" * 80)
    print("\nThis tool exports buildings with the highest potential for")
    print("thermal energy service (TES+HP) business development.")
    print("\nVersion 3 Fixed Features:")
    print("- Fixed property_type_clean error")
