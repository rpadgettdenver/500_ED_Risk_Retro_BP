"""
Suggested File Name: export_high_value_buildings_enhanced.py
...
class BusinessDevelopmentExporter:
    ...
    def export_high_value_targets(self):
        ...
        # Enhanced query with EUI targets
        query = f"""
        WITH building_details AS (
            SELECT 
                ...
                oda.building_name,
                oda.property_type,
                oda.property_type_clean,
                oda.gross_floor_area as sqft,
                oda.current_eui,
                
                -- EUI targets (from standards)
                s.target_2025,
                s.target_2027,
                s.target_2030,
            ...
            LEFT JOIN `{self.dataset_ref}.standards` s
                ON oda.property_type_clean = s.property_type_clean
        ),
        ...
        results_df = results_df.merge(
            epb_lookup[['Building ID', 'is_epb', 'epb_approved', 
                       'Original Final Target EUI', 'Approved Target Adjustment']],
            left_on='building_id',
        ...
        # Clean up columns
        results_df.drop('Building ID', axis=1, inplace=True)
        ...
        print(f"✓ Found {len(results_df)} high-value targets, {epb_count} are EPBs")
    ...
    def export_to_excel(self, df):
        ...
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            ...
            if len(epb_df) > 0:
                epb_df.to_excel(writer, sheet_name='EPB Buildings - Priority', index=False)
...
def main():
    ...
    print("🏢 ENERGIZE DENVER - HIGH VALUE BUILDING EXPORT (ENHANCED)")
    print("=" * 80)
    print("\nThis tool exports buildings with the highest potential for")
    print("thermal energy service (TES+HP) business development.")
    print("Now includes EPB data integration and EUI target details!\n")
    ...
    if response.lower() == 'y':
        ...
        top10 = df.head(10)[['overall_rank', 'building_name', 'property_type', 
                            'sqft', 'is_epb', 'current_eui', 'target_eui_2030',
        ...
        for _, row in top10.iterrows():
            ...
            print(f"   EUI: {row['current_eui']} → {row['target_eui_2030']} (target)")
