"""
Suggested File Name: portfolio_risk_analyzer_improved.py
File Location: /Users/robertpadgett/Projects/01_My_Notebooks/500_ED_Risk_Retro_BP/src/analysis/
Use: Improved portfolio risk analyzer with better table formatting and 2032 column

This version includes:
- Wrapped text in table labels
- Additional Avg $/sqft 2032 column
- Better formatting for the summary statistics table
"""
...
class PortfolioRiskAnalyzer:
    ...
    def create_visualizations(self, scenarios: Dict[str, pd.DataFrame], 
                            output_dir: str = None):
        """Create comprehensive visualizations with improved table formatting"""
        ...
        # 6. Financial Impact Summary - IMPROVED TABLE
        ax6 = plt.subplot(2, 3, 6)
        ax6.axis('off')
        
        # Create summary table with wrapped labels
        summary_data = []
        for name, df in scenarios.items():
            total_sqft = df['sqft'].sum()
            buildings_at_risk_2025 = (df['penalty_2025'] > 0).sum() if 'penalty_2025' in df.columns else 0
            avg_penalty_per_sqft_2030 = df['penalty_2030'].sum() / total_sqft if 'penalty_2030' in df.columns and total_sqft > 0 else 0
            avg_penalty_per_sqft_2032 = df['penalty_2032'].sum() / total_sqft if 'penalty_2032' in df.columns and total_sqft > 0 else 0
            ...
            summary_data.append([
                ...
                f"${avg_penalty_per_sqft_2030:.3f}",
                f"${avg_penalty_per_sqft_2032:.3f}"
            ])
    ...
    def create_executive_summary(self, scenarios: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        for name, df in scenarios.items():
            ...
            # Average penalties
            avg_2030 = df['penalty_2030'].sum() / total_sqft if 'penalty_2030' in df.columns else 0
            avg_2032 = df['penalty_2032'].sum() / total_sqft if 'penalty_2032' in df.columns else 0
            
            summary_rows.append({
                'Scenario': name.replace('_', ' ').title(),
                'Total Buildings': len(df),
                'Total Sq Ft': f"{total_sqft:,.0f}",
                'NPV of Penalties': f"${npv_total:,.0f}",
                'Buildings at Risk 2025': at_risk_2025,
                'Buildings at Risk 2030': at_risk_2030,
                'Buildings at Risk 2032': at_risk_2032,
                'Avg $/sqft 2030': f"${avg_2030:.3f}",
                'Avg $/sqft 2032': f"${avg_2032:.3f}",
...
if __name__ == "__main__":
    ...
    output_path = '/Users/robertpadgett/Projects/01_My_Notebooks/500_ED_Risk_Retro_BP/outputs/data/portfolio_risk_analysis_improved.json'
